...
@section('header')
    <div class="flex items-center justify-between">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Modifier le partenaire :') }} <span class="text-blue-600">{{ Str::limit($partner->name, 50) }}</span>
        </h2>
            ...
            <form method="POST" action="{{ route('admin.partners.update', $partner) }}" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div class="md:col-span-2 space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name', $partner->name) }}" required autofocus class="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md @error('name') border-red-500 @enderror">
                            @error('name')
                        ...
                        <div>
                            ...
                            <input type="url" name="website_url" id="website_url" value="{{ old('website_url', $partner->website_url) }}" placeholder="https://www.exemple.com" class="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md @error('website_url') border-red-500 @enderror">
                            @error('website_url')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="type" id="type" value="{{ old('type', $partner->type) }}" placeholder="Ex: Université, Entreprise, Institution" class="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md @error('type') border-red-500 @enderror">
                            @error('type')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="md:col-span-1 space-y-6">
                        ...
                        <div>
                            ...
                            <input type="number" name="display_order" id="display_order" value="{{ old('display_order', $partner->display_order) }}" min="0" class="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md @error('display_order') border-red-500 @enderror">
                        ...
                        <div class="flex items-start">
                            <div class="flex items-center h-5">
                                <input id="is_active" name="is_active" type="checkbox" value="1" {{ old('is_active', $partner->is_active) ? 'checked' : '' }} class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded">
                            </div>
                ...
                <div class="pt-8 mt-8 border-t border-gray-200 flex justify-end">
                    ...
                    <button type="submit" class="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                        {{ __('Mettre à jour le partenaire') }}
                    </button>
                </div>
            </form>
        </div>
