...
@section('title', __('Modifier la Page Statique'))

@section('header')
    <div class="flex flex-wrap justify-between items-center gap-4">
        <h1 class="text-2xl font-semibold text-gray-800 dark:text-white leading-tight">
            {{ __('Modifier la Page Statique') }}: 
            <span class="font-normal">{{ $staticPage->getTranslation('title', $availableLocales[0] ?? config('app.locale', 'fr'), false) }}</span>
        </h1>
        <div>
            <a href="{{ route('admin.static-pages.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-600 text-white text-sm font-medium rounded-md hover:bg-gray-700 shadow-sm transition ease-in-out duration-150 mr-2">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1.5" viewBox="0 0 20 20" fill="currentColor">
                    <path fill-rule="evenodd" d="M9.707 16.707a1 1 0 01-1.414 0l-6-6a1 1 0 010-1.414l6-6a1 1 0 011.414 1.414L5.414 9H17a1 1 0 110 2H5.414l4.293 4.293a1 1 0 010 1.414z" clip-rule="evenodd" />
                </svg>
                {{ __('Retour à la Liste') }}
            </a>
            @if($staticPage->is_published && Route::has('public.page') && !in_array($staticPage->slug, ['presentation-crpqa', 'appel-collaboration']))
                {{-- La condition !in_array(...) est spécifique à votre ancien code, à évaluer si toujours nécessaire --}}
    ...
    <div class="px-4 py-5 sm:p-6 md:p-8">
        ...
        <form action="{{ route('admin.static-pages.update', $staticPage->id) }}" method="POST" enctype="multipart/form-data">
            ...
            @include('admin.static_pages._form', [
                ...
                // 'users' => $users, // Décommentez si vous avez un champ de sélection pour user_id
            ])
