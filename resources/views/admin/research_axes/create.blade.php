...
@section('header')
    <div class="flex items-center justify-between">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Ajouter un nouveau Domaine de Recherche') }}
        </h2>
...
@section('content')
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 md:p-8 border-b border-gray-200">
            <form method="POST" action="{{ route('admin.research-axes.store') }}" enctype="multipart/form-data">
                @csrf
                <div class="grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-6">
                    ...
                    <div class="md:col-span-2 space-y-6">
                        <div>
                            ...
                            <input type="text" name="name" id="research_axis_name" value="{{ old('name') }}" required autofocus class="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md @error('name') border-red-500 @enderror">
                            @error('name') <p class="text-sm text-red-600 mt-1">{{ $message }}</p> @enderror
                        </div>

                        <div>
                            <label for="slug" class="block text-sm font-medium text-gray-700">{{ __('Slug (URL)') }} <span class="text-red-500">*</span></label>
                            <input type="text" name="slug" id="research_axis_slug" value="{{ old('slug') }}" required class="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md bg-gray-50 @error('slug') border-red-500 @enderror">
                        ...
                        <div>
                            ...
                            <input type="text" name="subtitle" id="subtitle" value="{{ old('subtitle') }}" class="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md @error('subtitle') border-red-500 @enderror">
                    ...
                    <div class="md:col-span-1 space-y-6">
                        <div>
                            ...
                            <input type="text" name="icon_class" id="icon_class" value="{{ old('icon_class') }}" placeholder="ex: planet-outline" class="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md @error('icon_class') border-red-500 @enderror">
                        ...
                        <div>
                            ...
                            <img id="research_axis_image_preview" src="#" alt="Aperçu de l'image" class="mt-2 max-h-40 w-auto rounded shadow-sm" style="display:none;"/>
                        ...
                        <div>
                            ...
                            <input type="number" name="display_order" id="display_order" value="{{ old('display_order', 0) }}" min="0" class="mt-1 block w-40 shadow-sm sm:text-sm border-gray-300 rounded-md @error('display_order') border-red-500 @enderror">
    ...
    {{-- Assurez-vous que les ID sont 'research_axis_name', 'research_axis_slug', 'research_axis_cover_image', 'research_axis_image_preview' --}}
@endsection