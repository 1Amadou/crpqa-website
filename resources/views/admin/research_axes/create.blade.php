...
@section('title', __('Ajouter un Axe de Recherche'))

@section('header')
    <div class="flex flex-wrap justify-between items-center gap-4">
        <h1 class="text-2xl font-semibold text-gray-800 dark:text-white leading-tight">
            {{ __('Ajouter un Nouvel Axe de Recherche') }}
        </h1>
        <a href="{{ route('admin.research-axes.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-600 text-white text-sm font-medium rounded-md hover:bg-gray-700 shadow-sm transition ease-in-out duration-150">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1.5" viewBox="0 0 20 20" fill="currentColor">
                <path fill-rule="evenodd" d="M9.707 16.707a1 1 0 01-1.414 0l-6-6a1 1 0 010-1.414l6-6a1 1 0 011.414 1.414L5.414 9H17a1 1 0 110 2H5.414l4.293 4.293a1 1 0 010 1.414z" clip-rule="evenodd" />
            </svg>
            {{ __('Retour à la Liste des Axes') }}
        </a>
    </div>
@endsection
...
<div class="bg-white dark:bg-gray-800 shadow-xl sm:rounded-lg">
    <div class="px-4 py-5 sm:p-6 md:p-8">
        <form action="{{ route('admin.research-axes.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            @include('admin.research_axes._form', [
                'researchAxis' => $researchAxis, // $researchAxis est une nouvelle instance passée par le contrôleur
            ...
            <div class="flex items-center justify-end mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
                ...
                <button type="submit" class="px-6 py-2.5 bg-primary-600 text-white font-semibold text-xs leading-tight uppercase rounded shadow-md hover:bg-primary-700 hover:shadow-lg focus:bg-primary-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-primary-800 active:shadow-lg transition duration-150 ease-in-out">
                    {{ __('Enregistrer l\'Axe de Recherche') }}
                </button>
            </div>
        </form>
    </div>
...
{{-- Le script pour les onglets est poussé par _form.blade.php --}}
{{-- Assurez-vous que TinyMCE est initialisé globalement ou poussez le script ici aussi si nécessaire --}}