...
@section('header')
    <div class="flex items-center justify-between">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Ajouter un nouvel événement') }}
        </h2>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-6">
                    ...
                    <div class="md:col-span-2 space-y-6">
                        ...
                        <fieldset class="space-y-4">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
                                ...
                                <div>
                                    ...
                                    <input type="date" name="start_datetime_date" id="start_datetime_date" value="{{ old('start_datetime_date') }}" required class="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md @error('start_datetime_date') border-red-500 @enderror">
                                ...
                                <div>
                                    ...
                                    <input type="time" name="start_datetime_time" id="start_datetime_time" value="{{ old('start_datetime_time') }}" required class="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md @error('start_datetime_time') border-red-500 @enderror">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
                                ...
                                <div>
                                    ...
                                    <input type="time" name="end_datetime_time" id="end_datetime_time" value="{{ old('end_datetime_time') }}" class="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md @error('end_datetime_time') border-red-500 @enderror">
                        ...
                        <div>
                            ...
                            <input type="text" name="location" id="location" value="{{ old('location') }}" placeholder="Ex: Salle de conférence X, En ligne (Zoom)" class="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md @error('location') border-red-500 @enderror">
                            @error('location')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                         ...
                         <div>
                            ...
                            <input type="url" name="registration_url" id="registration_url" value="{{ old('registration_url') }}" placeholder="https://exemple.com/inscription-externe" class="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md @error('registration_url') border-red-500 @enderror">
                            <p class="mt-1 text-xs text-gray-500">{{ __('Laissez vide si vous utiliserez le système d\'inscription interne (une fois disponible). Remplissez si vous utilisez un lien externe (Eventbrite, Google Forms, etc.).') }}</p>
                    ...
                    <div class="md:col-span-1 space-y-6">
                                ...
                                @if(isset($partners) && $partners->count() > 0)
                                    @foreach($partners as $partner)
                                        <option value="{{ $partner->id }}"
                                            {{ (is_array(old('partner_ids')) && in_array($partner->id, old('partner_ids'))) ? 'selected' : '' }}>
                                            {{ $partner->name }}
                                        </option>
                                    @endforeach
                                @else
                        ...
                        <div>
                            ...
                            <input type="text" name="tags" id="tags" value="{{ old('tags') }}" placeholder="Ex: Technologie, Éducation, Santé..." class="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md @error('tags') border-red-500 @enderror">
                            <p class="mt-1 text-xs text-gray-500">{{ __('Séparez les tags par des virgules.') }}</p>
                            @error('tags')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <textarea name="meta_description" id="meta_description" rows="3" class="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md @error('meta_description') border-red-500 @enderror">{{ old('meta_description') }}</textarea>
